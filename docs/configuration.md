...
Browsers with Web Push support can subscribe from the **Notifications** control. On iPhone and iPad this requires iOS/iPadOS 16.4 or newer, Gripi installed on the Home Screen, and notification permission granted from the control's first tap. Completed assistant replies are delivered while the web app is closed after a one-minute grace period. If any Gripi client for the same user reads the target session during that grace period, or the session is open and focused when delivery begins, notifications for that reply are suppressed on all subscribed devices. Focus is held through a short-lived, in-memory gateway lease, so a disconnected client stops suppressing notifications after about 30 seconds. Suppression is best-effort based on the shared read state and focus known when delivery begins.
