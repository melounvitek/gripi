...
test("uses GRIPI_DESKTOP_URL when present", () => {
  assert.equal(
    gatewayUrl({ GRIPI_DESKTOP_URL: "https://pi.example.test/gateway" }, []),
...
test("uses --gateway-url before environment configuration", () => {
  assert.equal(
    gatewayUrl(
      { GRIPI_DESKTOP_URL: "http://env.example.test" },
...
test("uses environment configuration before persisted configuration", () => {
  assert.equal(
    gatewayUrl({ GRIPI_DESKTOP_URL: "https://env.example.test" }, [], "http://configured.example.test:4567"),
...
test("falls back to localhost for unsupported URL schemes", () => {
  assert.equal(
    gatewayUrl({ GRIPI_DESKTOP_URL: "file:///tmp/gateway.html" }, [], "http://configured.example.test:4567"),
    DEFAULT_GATEWAY_URL
  );
});
...
test("falls back to localhost when --gateway-url is invalid", () => {
  assert.equal(
    gatewayUrl({ GRIPI_DESKTOP_URL: "https://pi.example.test" }, ["electron", ".", "--gateway-url=not a url"]),
    DEFAULT_GATEWAY_URL
  );
});
