...
class LiveStreamingJsTest < Minitest::Test
  ...
  def test_renderer_collapses_long_plain_bash_output_live
    result = run_javascript(<<~JS)
      const { LiveMessageRenderer } = await import(#{module_url("live_message_renderer.js").to_json});
      const fragment = () => ({
        childNodes: [],
      ...
      const desktopCount = { textContent: "" };
      const mobileCount = { textContent: "" };
      const collapse = {
        ...
        querySelector(selector) {
          return {
            ...
            ".tool-output-hidden-count--desktop": desktopCount,
            ".tool-output-hidden-count--mobile": mobileCount
          }[selector];
        }
      };
      const body = {
      ...
      console.log(JSON.stringify({
        collapsible: collapse.dataset.toolOutputCollapsible,
        collapsed: collapse.dataset.collapsed,
        controlHidden: control.hidden,
        visibleLines: body.children[0].children.map((line) => line.textContent)
      }));
    JS

    assert_equal "true", result.fetch("collapsible")
    assert_equal "true", result.fetch("collapsed")
    assert_equal false, result.fetch("controlHidden")
    assert_equal (8..25).map { |index| "line #{index}" }, result.fetch("visibleLines")
  end

  def test_live_native_bash_cards_are_collapsible
    result = run_javascript(<<~JS)
      const { LiveMessageRenderer } = await import(#{module_url("live_message_renderer.js").to_json});
      class Element {
        constructor() { this.children = []; this.dataset = {}; this.attributes = {}; this.className = ""; this.textContent = ""; }
      ...
      renderer.liveOutput = new Element();
      renderer.conversationController = { followLiveOutput: () => false, afterLiveOutputChange() {} };
      renderer.liveMessageAlreadyRendered = () => false;
      renderer.renderMessageImages = () => {};
      let collapsible;
      renderer.renderToolTranscriptBody = (body) => { collapsible = body.parent.dataset.toolOutputCollapsible; };

      renderer.appendCompactMessage("bashExecution", "$ generate", "output", true, false, new Date(), {
        bashExecution: true,
        toolName: "bash",
        toolTranscript: true
      });
      console.log(JSON.stringify({ collapsible }));
    JS
